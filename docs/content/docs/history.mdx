...
You can pass a `HistoryConfig` to any **<u>Agent</u>** or **<u>Pool</u>**. If a pool receives a history, it overrides individual agent histories so all agents share the same conversation thread. History can be stored using backends such as in-memory, file, SQLite, PostgreSQL, Redis, or custom storage backends.
...
If the conversation grows beyond `max_context_messages`, the configured **<u>strategy</u>** (trim or summarize) is applied to keep the context window manageable.

### Persist Data

All updates are saved back to the **<u>storage backend</u>**, ensuring the conversation history is retained across sessions and future runs.
...
<Callout type="info"> `summarize_model` is used only with `"summarize"` and `"smart"` strategies. If not provided, the **<u>Agent</u>**'s model will be used. </Callout>
...
Learn more about Advance Features in **<u>[Advanced Features](/docs/advanced)</u>**.
